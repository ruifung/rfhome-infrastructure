...
function importConfigMap() {
  ...
  --dry-run \
  -o yaml \
  >> $MON_ENDPOINT_CONFIGMAP_NAME.yaml
}
...
function importCsiRBDNodeSecret() {
  ...
  --dry-run \
  -o yaml \
  >> $CSI_RBD_NODE_SECRET_NAME.secret.yaml
}
...
function importCsiRBDProvisionerSecret() {
  ...
  --dry-run \
  -o yaml \
  >> $CSI_RBD_PROVISIONER_SECRET_NAME.secret.yaml
}
...
function importCsiCephFSNodeSecret() {
  ...
  --dry-run \
  -o yaml \
  >> $CSI_CEPHFS_NODE_SECRET_NAME.secret.yaml
}
...
function importCsiCephFSProvisionerSecret() {
  ...
  --dry-run \
  -o yaml \
  >> $CSI_CEPHFS_PROVISIONER_SECRET_NAME.secret.yaml
}
...
function importRGWAdminOpsUser() {
  ...
  --dry-run \
  -o yaml \
  >> $RGW_ADMIN_OPS_USER_SECRET_NAME.secret.yaml
}

function createECRBDStorageClass() {
cat <<eof >> $RBD_STORAGE_CLASS_NAME.yaml
...
cat <<eof >> $CEPHFS_STORAGE_CLASS_NAME.yaml
apiVersion: storage.k8s.io/v1
kind: StorageClass
metadata:
